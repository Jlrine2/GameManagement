...
data "archive_file" "api_code" {
  ...
  output_path = ".dist/${var.name}-status.zip"
}

resource "google_storage_bucket_object" "code_archive" {
  name   = "${var.name}_status_${data.archive_file.api_code.output_md5}.zip"
...
resource "google_cloudfunctions_function" "function" {
  ...
  name         = "${var.name}_status"
  entry_point  = "get_server_status"
  description  = "function for ${var.name} status"
  runtime      = "python39"
  timeout      = 300
  trigger_http = true
